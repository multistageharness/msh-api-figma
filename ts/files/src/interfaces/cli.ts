...
function getSDK(options: any): FigmaFilesSDK {
  ...
  if (!apiToken) {
    ...
    console.error(
      "Set via --token flag or FIGMA_API_TOKEN environment variable",
    );
    console.error(
...
program
  ...
  .option(
    "--depth <n>",
    "How deep into the document tree to traverse",
    parseInt,
  )
  .option("--geometry <type>", 'Set to "paths" to export vector data')
  .option(
    "--plugin-data <ids>",
    "Comma-separated plugin IDs to include data for",
  )
...
program
  ...
  .action(async (options: any, command: any) => {
    ...
    try {
      const images = await sdk.service.renderImages(
        options.fileKey,
        options.ids,
        {
          version: options.version,
...
program
  ...
  .action(async (options: any, command: any) => {
    ...
    try {
      const matches = await sdk.searchNodesByName(
        options.fileKey,
        options.query,
      );
