...
fastify.get('/test', async (request: any, reply: any) => {
  ...
  try {
    ...
    const sdk = new FigmaDevResourcesSDK({ fetcher: new (FigmaApiClient as any)({ apiToken: FIGMA_TOKEN }) });
...
fastify.get('/dev-resources/:fileKey/stats', async (request: any, reply: any) => {
  if (!FIGMA_TOKEN) {
    return reply.code(400).send({
      success: false,
      error: 'FIGMA_TOKEN environment variable not set'
