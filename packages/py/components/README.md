...
```python
sdk = FigmaComponentsSDK(api_key="...", verify="/etc/ssl/corp-ca.pem")
```

`verify` accepts anything `httpx` accepts — `True`, `False`, a path to a CA bundle, or an
`ssl.SSLContext` you build yourself. It is available on `FigmaComponentsSDK` and `FigmaComponentsClient` alike.
...
```bash
figma-components components --ssl-verify        # force verification on, overriding the environment
figma-components components --no-ssl-verify     # turn it off (alias: --insecure)
```
