        ...
        **client_kwargs: Any,
        ...
        Args:
            ...
            **client_kwargs: Additional arguments for the underlying client.
                Notably ``verify`` — TLS certificate verification: ``True``,
                ``False``, a CA-bundle path, or an ``ssl.SSLContext``. Omitting
                it resolves from ``FIGMA_SSL_VERIFY`` and then to ``True``; it
                never means "off". See
