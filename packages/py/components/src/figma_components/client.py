...
from .utils import validate_api_key, build_query_params, resolve_verify
...
class FigmaComponentsClient:
    ...
    def __init__(
        ...
        **httpx_kwargs: Any,
        ...
        Args:
            ...
            verify: TLS certificate verification, forwarded to
                ``httpx.AsyncClient``. ``True`` to verify, ``False`` to disable,
                a path to a CA bundle, or an ``ssl.SSLContext``. ``None`` (the
                default) means *unspecified*, not *off*: it resolves from
                ``FIGMA_SSL_VERIFY`` and falls back to ``True``. See
        ...
        # Resolve TLS verification once, here, so the "disabled" warning fires
        # where the caller made the choice rather than on the first request.
        #
        # `verify` cannot also arrive via **httpx_kwargs: it is a named
        # parameter, so Python binds it here and raises "got multiple values for
        # keyword argument 'verify'" on any attempt to pass it twice. The key is
        # written after the **httpx_kwargs spread regardless, so the resolved
        # value is the one that reaches httpx.
        client_kwargs = {
    ...
    def verify(self) -> bool | str | ssl.SSLContext:
        ...
        Read-only: resolution happens once, in ``__init__``.
        """
