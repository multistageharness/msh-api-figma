...
# Tri-state TLS flags, shared by every command that builds a client.
#
# The default MUST stay None. A plain bool would make every invocation an
# explicit choice and silently defeat FIGMA_SSL_VERIFY — see
# msh-api-figma/v100/docs/ssl-verify-contract.md.
SSL_VERIFY_HELP = "Verify TLS certificates (default: on; env FIGMA_SSL_VERIFY)."
INSECURE_HELP = "Alias for --no-ssl-verify."


def _ssl_verify_option() -> Any:
    return typer.Option(None, "--ssl-verify/--no-ssl-verify", help=SSL_VERIFY_HELP)


def _insecure_option() -> Any:
    return typer.Option(False, "--insecure", help=INSECURE_HELP)


def resolve_ssl_flags(ssl_verify: Optional[bool], insecure: bool) -> Optional[bool]:
    """Collapse the two CLI flags into one tri-state value for the client.

    ``--insecure`` wins over ``--ssl-verify``, matching the downloader's
    precedence (``figma_downloader/config.py:94-99``). Untouched flags return
    None, which leaves FIGMA_SSL_VERIFY in effect.
    """
    if insecure:
        return False
    return ssl_verify
...
def get_sdk(
    ...
    Args:
        api_key: Figma API key; falls back to FIGMA_API_KEY.
        verify: Tri-state TLS verification. None defers to FIGMA_SSL_VERIFY.
    """
    import os
    ...
    return FigmaFileSDK(api_key=key, verify=verify)
...
def get_file(
    ...
    ssl_verify: Optional[bool] = _ssl_verify_option(),
    insecure: bool = _insecure_option(),
) -> None:
    """Get file information and structure."""
    async def _get_file() -> None:
        async with get_sdk(api_key, resolve_ssl_flags(ssl_verify, insecure)) as sdk:
            try:
        ...
        async with get_sdk(api_key, resolve_ssl_flags(ssl_verify, insecure)) as sdk:
            try:
                ...
                if output_dir:
                    ...
                    import httpx
                    # Same TLS policy as the API calls — resolved once by the
                    # client, not re-resolved here (that would warn twice).
                    async with httpx.AsyncClient(verify=sdk.client.verify) as client:
                        for node_id, url in images_data.images.items():
                            if url:
                                try:
    ...
    async def _render_node() -> None:
        async with get_sdk(api_key, resolve_ssl_flags(ssl_verify, insecure)) as sdk:
            try:
                ...
                if output_file:
                    import httpx
                    # Same TLS policy as the API calls — see render_images.
                    async with httpx.AsyncClient(verify=sdk.client.verify) as client:
                        try:
                            response = await client.get(url)
                            response.raise_for_status()
...
def serve(
    ...
    console.print(f"\n[bold]API Documentation:[/bold] http://localhost:{port}/docs")
    console.print(f"[bold]OpenAPI Schema:[/bold] http://localhost:{port}/openapi.json\n")
