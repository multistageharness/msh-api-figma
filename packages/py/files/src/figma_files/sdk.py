...
class FigmaFileSDK:
    ...
    def __init__(self, client: Optional[FigmaFileClient] = None, **kwargs: Any) -> None:
        ...
        Args:
            ...
            **kwargs: Arguments to pass to client constructor. Notably
                ``verify`` — TLS certificate verification, ``True``/``False``, a
                CA-bundle path, or an ``ssl.SSLContext``. Omitting it resolves
                from ``FIGMA_SSL_VERIFY`` and then to ``True``; it never means
                "off". See ``msh-api-figma/v100/docs/ssl-verify-contract.md``.
                Ignored when ``client`` is supplied — configure verification on
                the client you pass in.
        """
