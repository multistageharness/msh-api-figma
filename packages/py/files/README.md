...
```python
sdk = FigmaFileSDK(api_key="...", verify="/etc/ssl/corp-ca.pem")
```

`verify` accepts anything `httpx` accepts — `True`, `False`, a path to a CA bundle, or an
`ssl.SSLContext` you build yourself. It is available on `FigmaFileSDK` and `FigmaFileClient` alike.
...
```bash
figma-files get-file FILE_KEY --ssl-verify        # force verification on, overriding the environment
figma-files get-file FILE_KEY --no-ssl-verify     # turn it off (alias: --insecure)
```
