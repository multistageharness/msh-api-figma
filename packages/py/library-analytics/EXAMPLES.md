...
from figma_library_analytics import FigmaAnalyticsSDK


async def main():
    async with FigmaAnalyticsSDK(
        api_key="...",
...
sdk = FigmaAnalyticsSDK(api_key="...", verify=context)
```
...
```python
sdk = FigmaAnalyticsSDK(api_key="...", verify=False)
```

```bash
figma-analytics component-actions FILE_KEY --no-ssl-verify     # alias: --insecure
```
