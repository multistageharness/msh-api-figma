...
```python
sdk = FigmaAnalyticsSDK(api_key="...", verify="/etc/ssl/corp-ca.pem")
```

`verify` accepts anything `httpx` accepts — `True`, `False`, a path to a CA bundle, or an
`ssl.SSLContext` you build yourself. It is available on `FigmaAnalyticsSDK` and `FigmaAnalyticsClient` alike.
...
```bash
figma-analytics component-actions FILE_KEY --ssl-verify        # force verification on, overriding the environment
figma-analytics component-actions FILE_KEY --no-ssl-verify     # turn it off (alias: --insecure)
```
