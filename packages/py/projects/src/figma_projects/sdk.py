    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            verify: TLS certificate verification, forwarded to the client.
                ``True`` to verify, ``False`` to disable, a path to a CA bundle,
                or an ``ssl.SSLContext``. ``None`` (the default) means
                *unspecified*, not *off*: it resolves from ``FIGMA_SSL_VERIFY``
                and falls back to ``True``. See
