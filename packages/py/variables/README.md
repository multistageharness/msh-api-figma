...
```python
sdk = FigmaVariablesSDK(api_token="...", verify="/etc/ssl/corp-ca.pem")
```

`verify` accepts anything `httpx` accepts — `True`, `False`, a path to a CA bundle, or an
`ssl.SSLContext` you build yourself. It is available on `FigmaVariablesSDK` and `FigmaVariablesClient` alike.
...
```bash
figma-variables list-variables FILE_KEY --ssl-verify        # force verification on, overriding the environment
figma-variables list-variables FILE_KEY --no-ssl-verify     # turn it off (alias: --insecure)
```
