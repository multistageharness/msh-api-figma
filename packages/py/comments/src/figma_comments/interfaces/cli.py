...
def list(
    ...
    async def _list_comments():
        ...
        async with FigmaCommentsSDK(api_token, verify=resolve_ssl_flags(ssl_verify, insecure)) as sdk:
            with Progress(
                SpinnerColumn(),
...
def health(
    ...
    async def _health_check():
        ...
        async with FigmaCommentsSDK(api_token, timeout=timeout, verify=resolve_ssl_flags(ssl_verify, insecure)) as sdk:
            with Progress(
                SpinnerColumn(),
                TextColumn("[progress.description]{task.description}"),
                console=console,
