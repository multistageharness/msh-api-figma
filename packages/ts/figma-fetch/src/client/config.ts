...
/**
 * Redact credentials from a proxy URL for safe logging. A proxy URL frequently
 * carries `user:secret@` — log `redactProxy(egress.proxy)` rather than the raw
 * value. A string that does not parse as a URL is returned as-is; `null` /
 * `undefined` / `''` return `''`.
 */
export function redactProxy(proxy: string | null | undefined): string {
  if (!proxy) return "";
  try {
