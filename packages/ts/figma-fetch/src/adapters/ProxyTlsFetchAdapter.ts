...
export class ProxyTlsFetchAdapter extends FetchAdapter {
  ...
  async fetch<T = any>(request: FetchRequest): Promise<FetchResponse<T>> {
    ...
    // first aborts the in-flight request, and didTimeout() tells the two
    // apart so only a timeout maps to TimeoutError.
    const ctx = this.createTimeoutContext(transformed.signal, timeout);
    ...
    try {
      ...
      const { data, rawText } = await this.readResponseBody(raw, contentType);
      ...
      if (error?.name === "AbortError") {
        ...
        // Caller-triggered abort stays distinguishable from a timeout.
        throw error;
      }
