...
program
  ...
  .option(
    "-t, --token <token>",
    "Figma access token (or set FIGMA_TOKEN env var)",
  )
...
program
  ...
  .option(
    "-n, --node-ids <ids>",
    "Comma-separated list of node IDs to filter by",
  )
  ...
  .action(async (fileKey: string, options: any, command: Command) => {
    ...
    try {
      const nodeIds = options.nodeIds
        ? options.nodeIds.split(",").map((id: string) => id.trim())
        : null;
...
program
  ...
  .action(
    async (fileKey: string, nodeId: string, options: any, command: Command) => {
...
program
  ...
  .action(async (options: any, command: Command) => {
    ...
    try {
      ...
      const results = await (sdk as any).createMultiFileDevResources(
        resourcesData,
        progressCallback,
      );
  ...
  .action(async (id: string, options: any, command: Command) => {
    ...
    try {
      ...
      if (Object.keys(updates).length === 0) {
        throw new Error(
          "At least one update field (name or url) must be provided",
        );
      }
...
program
  ...
  .action(
    async (fileKey: string, id: string, options: any, command: Command) => {
      if (!options.yes) {
        console.log(
          chalk.yellow(
            `About to delete dev resource ${id} from file ${fileKey}`,
          ),
        );
...
program
  ...
  .action(async (fileKey: string, _options: any, command: Command) => {
    ...
    try {
      ...
      console.log(
        chalk.blue(`Nodes with resources: ${stats.nodesWithResources}`),
      );
