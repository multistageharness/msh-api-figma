...
```bash
npm install @internal/figma-webhooks @internal/figma-api-fetch
```

`@internal/figma-api-fetch` is a peer dependency (resolves to `../figma-fetch` in this workspace).

## Usage

```ts
import { FigmaApiClient } from '@internal/figma-api-fetch';
import { FigmaWebhooksSDK } from '@internal/figma-webhooks';
